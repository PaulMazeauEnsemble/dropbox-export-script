    ...
    try:
        ...
        for entry in result.entries:
            if isinstance(entry, dropbox.files.FileMetadata):
                # Récupère les fichiers MP3
                if entry.name.endswith('.mp3'):
                    link = dbx.files_get_temporary_link(entry.path_lower).link
                    project_name = os.path.splitext(entry.name)[0].split('_')[-1]
                    files_info['mp3'][project_name] = link
                    print(f"MP3 trouvé : {entry.name} - Projet : {project_name}")
                # Récupère l'image
                elif entry.name.endswith(('.jpg', '.jpeg', '.png')):
                    # Obtenir le lien téléchargeable de l'image
...
if __name__ == "__main__":
    ...
    folder_path = '/TAlents site/FEMMES/FEMMES ANG'
    
    talents_info = explore_folder(folder_path)
    generate_csv(talents_info, './results/talents_femmes_ang.csv')
